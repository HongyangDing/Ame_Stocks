...
def test_parser_exposes_prepare_and_the_four_execution_commands() -> None:
...
def test_stage_delta_fails_closed_when_real_adapter_is_absent(
    ...
    monkeypatch.setattr(
        cli,
        "_load_run_spec",
        lambda _root, _pin: SimpleNamespace(run_kind=I3ProductionRunKind.DELTA),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    with pytest.raises(cli.I3ProductionCliError, match="fail-closed"):
        cli._stage_delta(tmp_path, _pin("run-spec"))
