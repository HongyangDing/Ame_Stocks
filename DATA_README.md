...
### 2.1 2026-07-12 下载补齐前的 checkpoint-v5 全面审计基线

这次基线对补入 `daily_bars` 和 `legacy_financials` **之前**的 29 个数据集、56,242 个
...
24,468,470 行。新增两种数据的全量 Bronze 审计将使用新的最终报告；在最终报告落盘前，不能
把这组旧总数当成当前完整库存总数。checkpoint-v6 已验证 31 个数据集、58,771 manifests、
238,814 个 artifacts 和 60,920,727,431 bytes 压缩数据的物理完整性及正式计划覆盖，但也
暴露出统一合同错把官方 optional 的 `daily_bars.vw` 当成必填。第一次修正版 v7 因进一步发现
共享合同尚未强制名义 16:00 ET 而在初期主动终止，没有生成报告。完整修正后的最终
checkpoint-v8 预留路径为 `manifests/audits/bronze/full-2026-07-12-v8.json`；其实际状态与 SHA
只在报告完整落盘后填写。文件名中的 v6/v7/v8 是运行 checkpoint；最终报告内部 schema 为
v4，并明确说明 issue 总数是异质 violation instances，不是唯一坏行数。
