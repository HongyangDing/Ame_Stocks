...
条件 `id` 只在对应 data type 内唯一，候选键应至少包含 `(asset_class, data_type, id)`；一行
可以同时适用于多个 `data_types`，Silver 解码时应先展开该数组。
...
这是 SEC filing 总目录，一行对应一个 filing。候选主键为 `accession_number`。
