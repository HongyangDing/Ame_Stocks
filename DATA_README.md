...
实际历史行情下载了两套全市场每日 Flat File，并补了一套独立交付的非 OTC REST grouped
日线：

- 十年分钟聚合线，一天一个 gzip CSV。
- 十年日聚合线，一天一个 gzip CSV，用于快速 QA 和交叉核对。
- 十年非 OTC REST Daily Market Summary，一天一个不可变 gzip JSON；它与日线 Flat File
  独立交付，并在结果字段存在时提供全日 `vw`，用于跨产品核对而不是覆盖 Flat File。

其余数据不是更多的价格副本，而是构建可靠回测所需的参考数据、公司行动、做空、
SEC、新闻和宏观数据。特别是每天的 active + inactive 股票清单，用来避免只看到今天仍
存续股票所产生的幸存者偏差。

代码中仍保留逐 ticker `minute_bars` REST 适配能力，供少量 ticker 做抽样验证；十年分钟历史
没有再按 ticker 逐个通过 REST 重复下载。`daily_bars` 使用一次返回当日请求口径下有聚合
结果 ticker 的 grouped endpoint，已经按交易日完整保存，不是逐 ticker 抓取，也不是每日
...
完成 13-F、8-K、Ticker Overview、Condition Codes、非 OTC REST grouped 日线和 legacy financials
补充后，manifest 记录的 Bronze 压缩 payload 合计约 56.74 GiB；文件系统实时占用以审计
报告时的 `df` 为准。
...
| `daily_bars` | 2016-07-13 至 2026-07-09 | 2,511 个请求/页；24,460,838 行 | 709.82 MB | 完成；非 OTC grouped Daily Market Summary；`adjusted=false`（未按拆股调整） |
...
| `legacy_financials` | 2009-03-29 至 2026-07-09 | 18 个年度请求、3,784 页、377,576 行 | 393.05 MB | 完成；旧 combined endpoint，保留逐指标来源血缘 |
...
### 2.1 2026-07-12 Bronze 全面审计

checkpoint-v5 对补入 `daily_bars` 和 `legacy_financials` **之前**的 29 个数据集、56,242 个
manifest 和 232,519 个实际文件进行了独立全量校验：重新
计算 SHA-256、把每个 gzip 读到 EOF 以验证 CRC、解析 JSON/CSV、重算记录数、核对分页与正式
下载计划。对 v5 启动时已快照的文件，物理损坏、截断、hash/bytes/row mismatch、计划内漏项
和 partial 文件均为 0。扫描期间新下载的两种数据又增加了 6,295 个 payload artifacts，
因此 v5 留下一条 `orphan` 快照告警；这些路径都属于 `daily_bars` 或
`legacy_financials` 的正式产物，并非无 manifest 的孤儿文件，也不是文件损坏。
REST 声明的 205,944,660 行与重新解析行数完全一致；分钟和日线分别解析出 3,689,316,811 与
24,468,470 行。完整修正并使用严格 manifest 整数合同重跑的最终 checkpoint-v9 位于
`manifests/audits/bronze/full-2026-07-12-v9.json`，SHA-256 为
`a23fdd2aa4c613274dfe0dcca611e8ed1bd62153146f787ecd415c345c1a15d6`。它覆盖 31 个数据集、
58,771 manifests、238,814 个 artifacts、230,783,074 条 REST 记录、60,920,727,431 bytes
压缩数据和 116,628,775,208 bytes 原始数据；声明与重新解析记录数完全一致。
`authoritative_plan=passed`、`physical_integrity=passed`，对应 issue code 均为 0；总体状态
`failed` 只表示 `semantic_consistency=failed`，不是文件损坏。v6 的 optional `vw` 假阳性和
早期 v7 的名义 16:00 ET 合同问题均已修复。v8 是严格整数修复前的同结果 checkpoint，v9 与
它的 summary、gates、datasets 完全一致。报告内部 schema 为 v4；issue 总数是按页面、请求、
manifest 或聚合记录的异质 violation instances，不是唯一坏行数。

统一 REST semantic schema-v7 报告位于
`manifests/audits/rest_semantics/full-2026-07-12-v7.json`，SHA-256 为
`95366ec4abcdc9903b0c1aea972e2cf9f14da008f931bdfc3111523addfae301`。它覆盖 26 个数据集、
2,702 个完整权威 manifests、116,111 pages 与 157,947,737 行；预期 manifest missing/invalid
均为 0。其 hard findings 是 Float 1 行、legacy 47 个 row-contract findings 与 2 条 accession
日期差异；候选键歧义、精确重复和 legacy EDGAR coverage 差异属于显式诊断输入，不是漏页或
bit rot。
...
版本；13-F 有 152 条只有 filing metadata、holding 字段整组缺失的 HR/HR-A 记录。v5 基线的
accession 对账只覆盖 `form_3`、`form_4`、`form_13f`、`eight_k_text` 和
`eight_k_disclosures`：这些受检 accession 均能在 EDGAR 找到且 filing date 一致；Form 13-F
的 CIK/form type 也能由同一条 EDGAR identity row 精确见证。该结论不外推到当时未参加
accession gate 的其他 SEC 数据集；两套 taxonomy 也都可解码。
...
### 2.2 REST 日线与 Day Flat File 独立交叉审计
...
报告 SHA-256 是
`f0588ca0b1ac54dcd2d4883c010725cafe723d0931977200f5c8b0486d34c7fe`。它覆盖
2016-07-13 至 2026-07-09 的 2,511 个共同请求交易日：186 日完全通过、2,324 日存在产品差异、
1 日触发行级 source-integrity 异常。唯一 source-integrity 异常仍是 2019-08-12 Flat File 的
29 个非规范时间戳；同一 S3 对象独立重下 SHA 一致，因此报告总状态为 `failed`，但这不表示
本地文件损坏。

交叉核对结果为 24,452,482 个共同 ticker-date 行、8,356 个仅 REST 行和 64 个仅 Flat 行。
REST 共 24,460,838 行，其中 `vw` 和 `n` 字段都存在 24,317,162 行，缺任一字段 143,676 行；
这里统计的是字段存在性，且 `vw` 的口径是全日而不是开盘半小时。
在共同 ticker-date 上，容差外不一致数分别为：`open` 705、`high` 1,466、`low` 15,872、
`close` 4,784、`volume` 8,643,243、`transactions` 8,640,546。因而最终 gates 是
`source_integrity=failed`、`ticker_coverage=different`、
`numerical_reconciliation=different`。这些结果证明两个产品不能被静默混用或互相覆盖；
Silver 必须保留来源列，并对研究口径选择一个权威来源。
...
- 主要用途：计算日内特征、t+1 日 09:30–10:00 ET 的**分钟收盘价成交量加权代理**、检查
  停牌和分钟缺口。

Minute Flat File 没有逐笔成交价，也没有每分钟 `vw`，所以不能从这些字段精确还原真正的
09:30–10:00 成交 VWAP。当前可重复的代理定义是
`sum(minute_close * minute_volume) / sum(minute_volume)`；代码、页面和回测结果必须把它命名为
`minute_close_vwap_proxy` 一类明确的代理名称，不能标成真实 VWAP。全日 REST `daily_bars.vw`
同样不能替代这个 30 分钟窗口。若简历项目最终要求精确执行 VWAP，需要另行获取该窗口的
provider aggregate/trade 数据并独立验证。
...
### 4.3 `daily_bars`：REST Daily Market Summary

官方说明：[Daily Market Summary](https://massive.com/docs/rest/stocks/aggregates/daily-market-summary)。

本项目对每个 XNYS 交易日调用一次 grouped endpoint，固定
`adjusted=false, include_otc=false`。它不是逐 ticker 抓取；每个请求的 `results[]` 同时包含
provider 在该请求口径下为当日返回且有聚合结果的 ticker。它不是完整 security master，
也不能替代 `assets` 的每日 active + inactive universe。物理路径为：

```text
/mnt/HC_Volume_106309665/american_stocks/bronze/massive/daily_bars/request_id=<request_id>/page-00000.json.gz
```

- 粒度：一行是一个 ticker 的 provider 全日聚合。
- 分区：请求中的一个 `session_date` 对应一个 manifest 和一个 JSON gzip 页面。
- 候选键：`(requested_session_date, T)`；不能只凭 `t` 推断真实交易所收盘时刻。
- 主要用途：与 Day Flat File 独立交叉核对，并在字段存在时保留 provider 全日 `vw`。

当前 Key 对 2016-07-11 和 2016-07-12 的 fresh request 都稳定返回 HTTP 403，从
2016-07-13 起返回 200；因此 REST 产品的权威起点是 2016-07-13。前两日仍有完整 Flat File，
但不能伪造 REST 对照行，也不能把 entitlement 边界写成“市场无数据”。

响应外壳除通用字段外还包含：

| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `adjusted` | boolean | 当前固定为 `false`；官方语义是响应未按拆股调整，不是涵盖股息等所有公司行动的通用复权开关。 |
| `queryCount` | integer | 官方定义为用于生成响应的 aggregate 数。本次 2,511 个响应中实测它都等于 `resultsCount` 和解码后的 `results[]` 行数；审计把偏离这一已观察合同的响应标为来源异常，但不把等式误写成官方通用定义。 |
| `resultsCount` | integer | provider 声明的返回行数；审计要求与 `results[]` 行数一致。 |
...
| --- | --- | --- |
| `T` | string | ticker。 |
| `otc` | boolean，可选 | provider 的 OTC 标记；当前请求固定 `include_otc=false`，若字段出现仍原样保留并校验。 |
| `o` | number | provider 全日开盘价。 |
| `h` | number | provider 全日最高价。 |
| `l` | number | provider 全日最低价。 |
| `c` | number | provider 全日收盘价。 |
| `v` | number | provider 全日成交量。 |
| `vw` | number，可选 | provider 全日 VWAP；不是 09:30–10:00 的执行 VWAP。 |
| `n` | integer，可选 | provider 全日成交笔数。 |
| `t` | integer | Unix 毫秒；该产品实测为请求交易日 ET **名义 16:00** 的窗口结束标记。 |

`t` 的名义 16:00 规则也适用于半日市：例如交易所 13:00 ET 收盘时，REST 日线仍返回
16:00 ET，而 Day Flat File 的 `window_start` 是当日 00:00 ET。因此 `t` 不是交易所真实
close，也不是信息实际可用时刻；Silver 必须保留请求交易日、provider 时间和交易日历真实
close 三者，不能用 timestamp 的字面值决定半日市可交易边界。

全量 24,460,838 行的字段存在性扫描中，`vw` 和 `n` 都存在 24,317,162 行，至少缺一个的有
143,676 行。这个数字只说明字段是否存在；`vw` 即使存在也只是 provider 全日 VWAP，不是
09:30–10:00 的执行 VWAP。缺字段表示 provider 没有提供，不能补零；跨产品
OHLCV/transactions 差异以 §2.2 的最终 schema-v4 报告为准。
...
### 9.10 `legacy_financials`：旧 combined financials fallback

当前 Key 对新的 [Income Statements](https://massive.com/docs/rest/stocks/fundamentals/income-statements)、
[Balance Sheets](https://massive.com/docs/rest/stocks/fundamentals/balance-sheets)、
[Cash Flow Statements](https://massive.com/docs/rest/stocks/fundamentals/cash-flow-statements) 和
[Ratios](https://massive.com/docs/rest/stocks/fundamentals/ratios) endpoint 重新探测后仍全部返回
HTTP 403；但旧的 `/vX/reference/financials` combined endpoint 可访问。
本项目因此把它作为**隔离的 legacy fallback**，按 `filing_date` 升序、每年一个请求、
`limit=100, include_sources=true` 下载。它已覆盖 2009-03-29 至 2026-07-09，共 18 个年度请求、
3,784 页、377,576 行和 18,124,688 个逐指标对象。

```text
/mnt/HC_Volume_106309665/american_stocks/bronze/massive/legacy_financials/request_id=<request_id>/page-<sequence>.json.gz
/mnt/HC_Volume_106309665/american_stocks/manifests/massive/legacy_financials/<request_id>.json
```

这个旧 endpoint 已不在当前正式产品文档中，不能推断它与新 v1 endpoint 永久等价，也不能
用它声称已取得独立的 ratios 历史或精确 point-in-time shares/market cap。Bronze 原样保存，
Silver 必须保留 `legacy` 数据源标签。强候选键是
...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `cik` | string | 10 位零填充 issuer CIK；连接 EDGAR 和每日资产身份的主要锚点。 |
| `company_name` | string | filing 对应公司名称。 |
| `start_date` | string(date) | 财务覆盖期开始；不是可用时间。 |
| `end_date` | string(date) | 财务覆盖期结束；不是可用时间。 |
| `filing_date` | string(date) | provider 给出的 SEC filing 日期；无可靠精确时间时按当日收盘后可用。 |
| `acceptance_datetime` | string(datetime)/null，可选 | SEC acceptance 时间；缺失或与 filing date 冲突时必须回退到保守规则。 |
| `timeframe` | string | `annual`、`quarterly` 或 `ttm`。 |
| `fiscal_period` | string | 与 timeframe 配套的 `FY`、`Q1`–`Q4` 或 `TTM`。 |
| `fiscal_year` | string | provider 标准化的财年。 |
| `sic` | string | provider 给出的 SIC；允许空字符串，空值必须视为 coverage gap。 |
| `tickers` | array[string]/null | filing 对应 ticker 列表；不能替代 point-in-time `assets`。 |
| `source_filing_url` | string | 含 canonical SEC accession 的 filing URL。 |
| `source_filing_file_url` | string | 含同一 canonical accession 的原始 filing 文件 URL。 |
| `financials` | object | 一个或多个非空财务章节，每个章节按标准指标名映射指标对象。 |

`financials` 只观察到三张主要报表 `balance_sheet`、`cash_flow_statement`、
`income_statement`，外加 `comprehensive_income` 章节。每个章节是
`{metric_name: metric_object}`；指标值允许负数，但 18,124,688 个值全部是有限 int/float。

所有指标对象的共同字段是：

| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `label` | string | provider/filing 中的人类可读指标名。 |
| `order` | integer | provider 的展示顺序，不是时间顺序。 |
| `source` | string | 精确为 `direct_report`、`intra_report_impute` 或 `inter_report_derive`。 |
| `unit` | string | 指标单位，例如 currency、shares 或 ratio 口径。 |
| `value` | number | 有限数值；不能把缺失指标补成零。 |

三个 provenance 形态互斥，字段集合也不同：

| `source` | 数量 | 额外字段 | 研究含义 |
| --- | ---: | --- | --- |
| `direct_report` | 9,325,293 | `xpath: string` | 直接来自当前 filing 的 XBRL 路径；优先作为可审计原值。 |
| `intra_report_impute` | 7,258,958 | `formula: string` | provider 在同一报告内按公式补算；必须与直接值分层。 |
| `inter_report_derive` | 1,540,437 | `derived_from: array[accession]` | 由一个或多个其他 filing 推导；使用前要验证所有来源 filing 当时已公开。 |

全量深扫验证了所有 gzip/JSON 可解析、manifest 行数一致、分页 continuation 和 provider
request ID 没有冲突，也没有重复整页、精确重复整行、空财务章节或非法指标 provenance。
但数据语义并非“零异常”，Silver/Gold 必须处理以下 PIT 和 coverage 项：

- 39 行出现 `end_date > filing_date`，超前 30–325 天；这些行有直接未来数据风险，必须隔离，
  不能仅靠排序修复。
- 299,200 行没有 `acceptance_datetime`，只有 78,376 行提供；有时间的记录中，4,245 行的
  acceptance 日期早于 `filing_date`，18 行晚于它。时区转 ET 不能消除这些差异，使用前应与
  EDGAR 交叉核对；其余记录按 filing date 收盘后可用是更保守的默认。
- 125,812 行 `tickers=null`；这是 ticker 映射 coverage，不是财务值缺失，连接时应以 CIK 和
  point-in-time 资产身份为主。8 行 ticker 数组内部重复，共 15 个 excess item，Silver 做稳定
  去重但保留原始行。
- 2,322 行 `sic` 为空字符串；不能前向/后向填充为某个行业，更不能用当前 Overview SIC
  无条件回填整段历史。
- 对 `filing_date >= 2016-07-11` 的 236,249 行，EDGAR Index 覆盖 235,757 行；缺少的 492 行
  （480 个 accession）记为 coverage difference。已匹配行的 issuer CIK 冲突为 0，仅两个
  accession 的 filing date 相差一天：`0001477932-26-001755` 和
  `0001193125-26-132193`（legacy 为 2026-03-31，EDGAR 为 2026-03-30），应优先隔离核验。

SEC accession 前 10 位对应提交该 accession 的 filing identity，不保证等于财务记录根对象的
issuer CIK；
全量有 255,259 行两者不同。因此不能把 accession 前缀与根 CIK 不同误报为身份损坏，正确
做法是在 EDGAR 同 accession 的多行中寻找 issuer CIK 见证。
...
| `request.adjusted` | boolean | 请求是否要求 provider 做拆股调整；当前 Bronze 原始行情为 `false`。该字段不是股息等所有公司行动的统一复权开关。 |
...
当前状态：除明确排除的超大数据和 live access 返回 403 的四个新 financial v1 endpoint 外，
当前 Key 可访问且与日频因子/Barra 相关的正式 Bronze 清单已经保存；13-F 正式 41 个季度、
非 OTC REST grouped 日线和 combined `legacy_financials` 也已完成。legacy 数据提供三张主要
报表、额外的 `comprehensive_income` 章节及逐指标 provenance，但不等价于新的 ratios 或
精确 point-in-time shares/market cap。两个验证日已经
生成未复权 `silver_unadjusted`，以下十年全量转换、复权和 Gold 尚未启动。

因此，普通日频价格/收益因子以及 price-derived Barra 风格（如 beta、动量、残差波动、
流动性）已经具备 Bronze 原料；vendor-grade 的经典 Barra 基本面风格仍不能宣称完整。
缺口主要是可审计的历史 shares/market cap 与 point-in-time 行业分类，不能用今天的 Float、
Ticker Overview market cap/SIC 或空 SIC 无条件回填十年历史。
...
7. 将 `legacy_financials` 的 direct/imputed/derived 指标分层；隔离 39 条日期倒置记录，并验证
   acceptance、EDGAR accession、CIK 及所有 `derived_from` filing 的可用时间。
8. 对 pilot、分页重试、修订 filing 和多 ticker 新闻做确定性去重，同时保留 lineage。

### Gold 计划

- 从 Silver minute bars 生成日频特征和 t+1 09:30–10:00 ET
  `minute_close_vwap_proxy`；除非另有精确窗口数据，不把它标成真实 VWAP。
...
| REST `daily_bars` | `t` 是名义 16:00 ET（半日市亦然），不是实际 close/available-at；`vw` 只是全日值。 |
...
| legacy financials | 优先使用核验后的 acceptance；否则按 filing date 收盘后可用。隔离 `end_date > filing_date`，并验证所有跨报告推导来源。 |
