...
完成本次 8-K 和 Ticker Overview 补充后，服务器文件系统共使用约 56 GiB，剩余约
131 GiB。
...
| `ticker_events` | 每个 identifier 的完整事件时间线 | 13,104 个事件 | 2.53 MB | 11,487 个请求完成；3,786 个请求失败并保留 manifest |
...
| `form_13f` | 2016-07-11 至 2026-07-09 | 约 26.79 百万行 | 约 1.37 GB | 下载仍在继续；此行会增长 |
...
`ticker_events` 的失败项是 identifier 级失败，不是成功文件损坏。失败 manifest 没有 artifact，
可以单独重试；它们不会影响其他 identifier 已完成的数据。`form_13f` 的行数包含已完成 pilot
和正在写入的季度流，Silver 必须按 filing/holding 语义去重，不能把这个即时总数当成最终
唯一记录数。
...
| `quotes` | 当前 Stocks Developer 权限实测为 403，且规模远大于 aggregates。 |
| Financial statements / ratios | 当前账号实测为 403，需要更高套餐或额外 Financials 权限。 |
...
当前状态：Bronze 原始层已经保存；13-F 仍在继续。两个验证日已经生成未复权
