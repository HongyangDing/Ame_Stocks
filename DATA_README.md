...
完成 13-F、8-K、Ticker Overview 和 Condition Codes 补充后，服务器文件系统共使用约
57 GiB，剩余约 129 GiB。
...
| `assets` active + inactive | 2016-07-11 至 2026-07-09 | 69,381,182 行；每天两次请求 | 2.53 GB | 完成 |
...
| `float` | 2026-07-09 当前快照 | 6,649 行 | 90.43 KB | 完成；非历史序列 |
| `ipos` | 2008-01-01 至 2026-07-09 | 5,492 行 | 273.51 KB | 完成 |
| `ticker_events` | 每个 identifier 的完整事件时间线 | 所有成功响应共 13,104 个事件 | 2.53 MB | 正式计划 11,471 个完成、3,702 个确认 HTTP 404；另有 100 个审计 pilot |
...
| `ten_k_sections` | 2016-07-11 至 2026-07-09 | 153,194 行 | 2.93 GB | 完成 |
| `eight_k_text` | 2016-07-11 至 2026-07-09 | 458,067 行 | 416.58 MB | 完成 |
| `eight_k_disclosures` | 实际返回 2022-01-03 至 2026-07-09 | 338,778 行 | 49.28 MB | 完成；请求的 2016–2021 年为空响应 |
| `disclosure_taxonomy` | 2026-07-09 当前字典 | 119 行 | 6.00 KB | 完成；非历史序列 |
| `risk_factors` | 2016-07-11 至 2026-07-09 | 627,674 行 | 68.08 MB | 完成 |
...
表中只列 41 个正式季度；额外 pilot 保持隔离。Silver 仍必须按 filing/holding 语义处理修订
和行级重复，不能把 Bronze 原始行数直接当成最终唯一持仓数。
...
4,853 组 inactive 版本行；Float 有一行缺 ticker；EDGAR Index 有 22,032 个精确重复 excess
rows 和 6,148 个候选 metadata 版本；Day Aggregate 与分钟线自行聚合存在产品口径差异。完整
方法、精确计数、证据路径和 Barra 完备性结论见
...
对每个交易日分别请求 `active=true` 和 `active=false`，合并后得到当日 provider 可见的完整
美股 ticker 集合。这是避免幸存者偏差的核心输入，而不是行情数据。
...
8-K、Form 3 和 Form 4 accession 均能在 EDGAR Index 中找到。
...
一行是机构 13-F information table 的一个持仓项，不是一份 filing。候选去重键必须包含
`accession_number`、CUSIP、证券类别、put/call、discretion 等行级字段或直接使用规范化行
hash。13F 修订表需要另外定义替换/追加语义。

| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `accession_number` | string | SEC filing accession number。 |
| `filer_cik` | string | 申报机构零填充 CIK。 |
| `form_type` | string | 如 `13F-HR`、`13F-HR/A`。 |
...
| `put_call` | string，可选 | `PUT`、`CALL`；普通股票通常缺失。 |
| `investment_discretion` | string | `SOLE`、`SHARED` 或 `DFND` 等投资裁量类型。 |
...
13-F 通常在季度末之后才提交。任何 crowding/机构持仓因子都必须按 `filing_date` 逐份进入，
...
| Financial statements / ratios | 官方显示 Stocks Advanced 应有权限，但当前远程 Key 对四个新 v1 endpoint 均返回 403；下载适配器已就绪，等待 Massive 刷新 entitlement。 |
...
当前状态：已授权且在正式清单内的 Bronze 原始层已经保存，13-F 正式 41 个季度也已完成。
三张历史财务报表和当前 ratios 快照因 entitlement 403 尚未保存。两个验证日已经生成未复权
`silver_unadjusted`，但以下十年全量转换、复权和 Gold 尚未启动。
