...
代码现在是 schema v7：在旧规则基础上加入 13-F 的同一 EDGAR row 见证；Daily 纳入 optional
字段、名义 16:00 ET 与候选键合同；zero-row terminal response 可安全省略 optional `results`；
legacy financials 的候选键使用 canonical accession，并纳入字段合同及 accession lineage。
语义审计有意只覆盖 29 个正式 REST 数据集中的 26 个；Assets、Ticker Overview 与 Ticker Events 由全量
...
.venv/bin/ame-audit-rest-semantics \
  --data-root /mnt/HC_Volume_106309665/american_stocks \
  --start 2016-07-11 --end 2026-07-09 \
  --output manifests/audits/rest_semantics/full-2026-07-12-v7.json
```
