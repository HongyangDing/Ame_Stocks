# Silver S2 `ticker_types` approved schema, preview, and published release

## 1. 当前状态与硬边界

2026-07-13，用户逐字批准精确 contract
`b2297d0631ae7560e7c3a9f73a288c62154db36b3188275e62f69c642884e38d`，随后另行明确批准只运行
当前 24 行 bounded preview，并在 review 后以“继续吧，把S2推进结束”明确授权 full build 与
publish。S2 现处于 **Phase 1 / `published`（sequence 9）**。Bronze 只读画像、字段语义、17 字段
schema、20 项 QA、manifest-bound source reader、纯转换、review-bound full build 和 release 均已
冻结并验收。
...
- 复验完整 workflow trust chain、source/artifact checksum、输出权限和 bounded replay 幂等性，然后
  在 `awaiting_review` 停止并展示；
- 在用户单独批准后，只把这一份 reviewed preview 重算为 full build，经无 waiver、无 quarantine
  acceptance 的 full-run 与 publish 两次审批发布；
- 对最终 `published` 状态做精确重放，并复验 Bronze、preview、S1 和 S3 边界均未改变。

最初 bounded-preview 指令只授权六个 `staging/silver` 人工 review 文件，并未授权 full 或 publish；
后续“继续吧，把S2推进结束”才是独立的 completion authorization。正式 release 只暴露 full build
中的一个 DATA Parquet，不把 preview 当作正式数据。S2 至此结束；该授权**不包括 S3**，也没有
读取或修改 S4 `assets`。下一硬停点在 S3 schema review 之前。
...
### 1.1 Packaged contract、bounded preview 与 release 模块
...
- 只允许精确 reviewed preview、用户 completion authorization 与不可变 provenance 的 release
  runner：[`ticker_type_release.py`](../backend/ame_stocks_api/silver/ticker_type_release.py)
- synthetic contract/transform 与 preview fail-closed tests：
  ...
  [`test_silver_ticker_type_release.py`](../tests/test_silver_ticker_type_release.py)
...
## 7. 已批准的精确决定与 completion authorization
...
Schema approval 没有自动授权真实数据运行；bounded preview 在另一条明确指令下运行，并先停在
`awaiting_review`。用户检查后又以“继续吧，把S2推进结束”单独授权以下两项：

1. 把精确 preview build/SHA 作为 full-run approval 的 subject，且不接受任何 QA waiver 或
   quarantine issue；
2. 只在 full build 与 preview 的 schema、row funnel、QA core、quarantine 和 DATA SHA 完全一致
   后发布，并把 publish approval 绑定到精确 full build/SHA。

该 authorization 文本、preview transform commit 和 release runner commit 均写入不可变 runtime
provenance；它不授权 S3 或任何其他 dataset。

## 8. Bounded preview 与发布结果

后续获批的真实 preview 只处理权威 manifest 中的当前 24 行，并按原指令先在
`awaiting_review` 停止：
...
### 8.1 Review-bound full build 与正式 release

用户 review 后单独批准 S2 继续到结束。最终 workflow 证据为：

| Evidence | ID / 结果 |
| --- | --- |
| final state/event | `published`，sequence 9；`006381bec6ee7c57ba43f2ace9f57cafc24006fb13b19dba0d1bdd749bc9952e` |
| full build | `f02a6ad085e5f78ac15f3d1e26caf75079275204e7b55b58b4bb679bdfab2780` |
| full build manifest SHA-256 | `cc2cd682da2ecf79665c7be3597726f517f8fa676ef1226d013f0d87c73ebac9` |
| full-run approval | `bcfceae023477ad4f910ce74ff649fe91bdd1ead82138e129eb152fbaed9f716`；0 QA waiver，0 quarantine acceptance |
| publish approval | `b5d3a032662fef9264fcbbcaedceb95e9b5dc5c3906eff626a224586e6e967ae`；0 QA waiver，0 quarantine acceptance |
| release | `11a62f9c06ea5c609c159a7d619ba94cabbe39d3b07518fec279fa4758c882f6` |
| release manifest SHA-256 | `5568a905bb1cdfe791a300f5b12fdd1e2041e3e1c1aacfbf6cc78f4890b95f47` |
| row funnel / schema | 24 input → 24 accepted → 24 output；17 columns；0 duplicate excess；0 quarantine |
| QA | 20/20 passed；三项 earliest-capture temporal QA 均为 numerator/denominator 0/0 |
| DATA SHA-256 | preview 与 full 均为 `8b3512e293edbfd5b6d813851720a9b5cb69dc212ab1ee3376daeee7f0fc6f11` |
| provenance commits | preview transform `8ad55c3f208ef3ff0de5e6666cfaa7491c1c3745`；release runner `463cedf64d6e9d71f2856d77c195c1510f836273` |

Full build 包含七个不可变输出：一个 DATA Parquet、QA Parquet、空 quarantine Parquet、完整
input/output samples、current-reference fixed-case evidence 和 runtime provenance。七个文件的实际
file set、bytes、SHA-256、row count、`0444` mode 与 `nlink=1` 均和 full manifest 一致。Runtime
provenance 记录精确 completion authorization、reviewed logic closure blobs、preview transform
commit、release runner commit 与运行库版本。Release manifest 只暴露其中一个 DATA artifact，
release-only reader 已重新验证完整 workflow chain。

发布前后，Bronze manifest/page 与六个 preview 文件的 SHA、bytes、mtime、inode、mode 和 link
metadata 均未改变。使用最终 published event 重放精确命令后，CLI JSON 与独立 verification JSON
逐字节相同，workflow 仍为 sequence 9，full/release/approval IDs、所有正式输出的 SHA、mtime 和
inode 也未改变。S1 仍保持原 published workflow sequence 9；S3 未创建 workflow、build、release
或正式输出。

**S2 已完成。下一顺序硬停点在 S3 `condition_codes` schema review 之前；本次授权不自动推进
S3。**
