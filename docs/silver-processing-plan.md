...
截至 2026-07-13，Silver 已按数据集推进到 S4，但不能描述成“全部 Silver 数据已处理”：

- S1 exchanges、S2 ticker types 和 S3 condition codes 已正式发布；
- S4 Assets 的 2026-05-11 bounded preview 已停在三条 `awaiting_review` chain，十年 full scope
  尚未创建 `FullRunPlan` 或获得批准；
...
- 旧 universe 代码会拒绝单个快照内的重复 ticker；新的 S4 runner 已在 bounded preview 保留版本，
  但不能把该单日授权直接扩成十年全量；
- 复权、最终身份、PIT availability、SEC/财务、新闻和宏观的正式 Silver 转换尚未实现；
- 旧 `ame-materialize`/`ame-flatfiles convert` 仍只生成历史 pilot；正式路径已由独立
  `ame-silver` 合同和 release-only reader 隔离，S1–S3 已接入并发布，S4 只有 staging preview。
...
**状态（2026-07-13）：已获批准、实现并验收。** S0 最初只在临时合成 fixture 上验证；S1–S4
...
S0 控制面已完成，S1、S2 与 S3 均已完成并发布。S3 的真实 94 行 `condition_codes` 已通过双 workflow 的
preview、review-bound full build、release 与精确重放。S4 已完成全量只读 source profile、三合同
批准、remote registry、manifest-bound reader、纯转换、synthetic QA 和获批单日 preview；三个
workflow 均停在 `awaiting_review` sequence 5。当前顺序硬停点是人工检查三个 preview，尚未批准
FullRunPlan、full build 或 publish。
...
**S2 已结束；S3 现已独立完成，S4 当前停在 `awaiting_review`，尚未运行 full build。**
...
**S3 至此结束；S4 已独立完成 bounded preview 并停在 `awaiting_review`，没有 full build 或 publish。**
...
**状态（2026-07-13）：Phase 2 / 三个 workflow 均 `awaiting_review`（sequence 5）。** 已只读扫描全部
2,513 个 session、5,026 个 active/inactive manifests、72,038 pages 和 69,381,182 rows。三张精确
contract 已逐字批准、封装为 package resources 并登记远程 registry；manifest-bound reader、按
session 有界的纯转换、duplicate selection 与 synthetic QA 已实现。获批的 2026-05-11 完整
active/inactive pair 已生成不可变 SourceInventory 和三张 staging preview；没有创建 full build、正式
Silver 数据或 S4 release。完整证据见
...
三条 chain 都是
`planned → schema_review → code_ready → preview_ready → awaiting_review`，批准 receipt 无 QA
waiver 或 accepted quarantine issue。Shared SourceInventory 为
...
三表均为 input=accepted=35,647、`version_preserved_rows=82`、0 quarantine、0 blocking QA；
warning 仅保留需人工解释的 provider/identity diagnostics。相同参数和 sequence-5 event SHA 的重跑
成功且 build/event 均未变化。下一步是人工 review；preview 不允许直接扩成十年 full run，必须先
另建并单独批准 `FullRunPlan`。
...
- 边界说明：旧 materializer 遇到这些重复仍会报错，S4 只使用新的 version-preserving runner；
  bounded preview 通过不代表十年 full scope 已批准。
