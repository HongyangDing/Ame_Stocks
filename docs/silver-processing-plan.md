...
S0、S1、S2 与 S3 均已完成并发布。S3 的真实 94 行 `condition_codes` 已通过双 workflow 的
preview、review-bound full build、release 与精确重放。S4 已完成全量只读 source profile、三合同
批准、remote registry、manifest-bound reader、纯转换和 synthetic QA；三个 workflow 均停在
`code_ready` sequence 3。当前顺序硬停点是独立的 bounded-preview 授权。
...
**S2 已结束；S3 现已独立完成，S4 当前停在 `code_ready`，尚未运行 preview。**
...
**S3 至此结束；S4 已独立进入 `code_ready`，但没有 SourceInventory、preview 或数据写入。**
...
**状态（2026-07-13）：Phase 2 / 三个 workflow 均 `code_ready`（sequence 3）。** 已只读扫描全部
...
session 有界的纯转换、duplicate selection 与 synthetic QA 已实现。没有创建 Assets SourceInventory，
也没有读取真实 preview session、写 staging/Silver 或创建 build/release。完整证据见
...
三条 chain 都是 `planned → schema_review → code_ready`，批准 receipt 无 QA waiver 或 accepted
quarantine issue。下一步只能在新的明确授权下运行完整 active/inactive session pair 的 bounded
preview，并必须停在 `awaiting_review`。
