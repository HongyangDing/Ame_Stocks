# Silver S4 `assets` source profile and schema review

## 1. 当前状态与硬边界

2026-07-13，S4 正式从 `planned` 进入 **Phase 2 / `schema_review`**。本检查点只完成：

- 对十年 active/inactive Assets Bronze 做全量只读 streaming profile；
- 核对字段、类型、空值、时间关系、每日 active/inactive 互斥、重复版本和身份冲突；
- 冻结三张目标表的 Git candidate、grain、字段、键、PIT 限制、选择规则和 QA；
- 用 `TableContract` 确认候选文件可确定性解析并固定 digest。

本检查点**没有**：

- 在远程 Silver registry 登记 schema-v1 workflow；
- 把 candidate 复制成 approved package resource；
- 编写或运行 source transform、preview、full build 或 publish；
- 写入 Bronze、`staging/silver` 或正式 Silver 路径；
- 修改旧 materializer、Docker、Caddy 或 Mogikabu。

远程 registry 对同一 `domain/table/schema-v1` 的 contract digest 是不可变的。为了让 review 期间仍能
修改字段而不被迫升 `v2`，三个 candidate 在用户逐字批准 digest 前只保存在 Git 中，沿用 S1 的
Git-only schema-review 模式。

| Candidate | 字段 | QA | `contract_id` | Arrow schema digest |
| --- | ---: | ---: | --- | --- |
...
任何字段、顺序、类型、nullability、key、QA 或描述变更都会改变对应 `contract_id`，必须重新 review。
...
这是 lossless semantic staging：重复版本不丢，非法结构才 quarantine。69M rows 不能复用 S1–S3
一次性放入内存的 `SourceBatch(tuple(rows))`；批准后 source reader/transform 必须按 session/page
streaming，先在 preview 测真实 Parquet 体积和峰值内存。
...
三份 candidate 的精确 QA 列表在 JSON 中冻结。关键 Critical/High gate 包括：
...
因此它不进入 S4 正式路径，也不被原地放宽。批准 schema 后会实现新的 session/page streaming
reader 与纯转换。输出仍符合“每天一个逻辑文件/partition”的目标：Parquet 以
...
本轮停在 Git-only `schema_review`。用户若批准，必须同时逐字批准三个 contract ID；之后才能：

1. 把三个精确 candidate 封装为 approved package resources；
2. 登记三个独立、lockstep workflow 并推进到 `code_ready`；
3. 实现 streaming source reader、纯转换和 synthetic conflict fixtures；
4. 只运行按完整 session pair 选择的 bounded preview；
5. preview 写 `staging/silver` 后停在 `awaiting_review`，不自动 full/publish。
...
人工 review。Synthetic fixture 仍必须补 identity conflict、非 exact timestamp tie、case-sensitive ticker
reuse。是否再加入 earliest/first-duplicate/latest boundary day，在 schema 获批后的 preview
authorization 中精确冻结；本轮不读取 preview、不创建 runtime workflow。
