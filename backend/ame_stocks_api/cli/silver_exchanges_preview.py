"""Explicit mutating CLI for the bounded S1 exchanges preview only."""
...
from ame_stocks_api.silver.exchange_preview import run_exchange_preview
from ame_stocks_api.silver.store import SilverStoreError


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="ame-silver-exchanges-preview",
        description=(
            "Build one manifest-bound exchange_dim preview and stop at awaiting_review. "
    ...
    parser.add_argument("--actor", default="s1-exchanges-preview-runner")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if preview is None:  # pragma: no cover - BuildManifest enforces this
            raise SilverStoreError("registered exchange preview has no preview metadata")
        output = {
        ...
        parser.exit(2, f"ame-silver-exchanges-preview: {exc}\n")
