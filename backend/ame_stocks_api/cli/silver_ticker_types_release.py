"""Explicit CLI for completing the exact reviewed S2 ticker-types workflow."""
...
from ame_stocks_api.silver.ticker_type_release import complete_ticker_type_release


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="ame-silver-ticker-types-release",
        description=(
            "Advance only the exact reviewed 24-row ticker_type_dim preview through a "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        run = complete_ticker_type_release(
            arguments.data_root,
        ...
        parser.exit(2, f"ame-silver-ticker-types-release: {exc}\n")
