"""Build and register the bounded S2 ticker_type_dim preview, never a full build."""
...
CURRENT_TICKER_TYPES_MANIFEST_PATH = (
    "manifests/massive/ticker_types/"
    "b1e581dac57b064039555580a56d6179b8ecf3a3d00dce7e2ade8cf8abc6dea6.json"
)
CURRENT_TICKER_TYPES_MANIFEST_SHA256 = (
    "14e997a8ffd89ee5061bdf6d8c63db1974a9e257b2bb8c3b42d2f08bb3952825"
)
CURRENT_TICKER_TYPES_REQUEST_ID = "b1e581dac57b064039555580a56d6179b8ecf3a3d00dce7e2ade8cf8abc6dea6"
CURRENT_TICKER_TYPES_ARTIFACT_PATH = (
    f"bronze/massive/ticker_types/request_id={CURRENT_TICKER_TYPES_REQUEST_ID}/page-00000.json.gz"
)
CURRENT_TICKER_TYPES_ARTIFACT_SHA256 = (
    "b074aea89befa8bc6795bbd10c34d86448e32b7dec39708a2d4a9983b26e6af6"
)
CURRENT_TICKER_TYPES_EXPECTED_ROWS = 24
TICKER_TYPE_PREVIEW_POLICY_VERSION = "ticker-type-preview-v1"
...
_TEMPORAL_FIXED_CASE_QA_CHECKS = (
    "description_changed_rows_since_prior_capture",
    "disappeared_type_code_rows_since_prior_capture",
    "new_type_code_rows_since_prior_capture",
)
...
class TickerTypePreviewAuthorization:
    ...
    def __post_init__(self) -> None:
        if not self.manifest_path.startswith("manifests/massive/ticker_types/"):
            raise ValueError("preview authorization manifest path is outside ticker_types")
        if not self.artifact_path.startswith("bronze/massive/ticker_types/"):
            raise ValueError("preview authorization artifact path is outside ticker_types")
        for label, value in (
        ...
        if type(self.sample_limit) is not int or not 1 <= self.sample_limit <= 100:
            raise ValueError("preview authorization sample_limit must be between 1 and 100")


CURRENT_TICKER_TYPES_PREVIEW_AUTHORIZATION = TickerTypePreviewAuthorization(
    manifest_path=CURRENT_TICKER_TYPES_MANIFEST_PATH,
    manifest_sha256=CURRENT_TICKER_TYPES_MANIFEST_SHA256,
    request_id=CURRENT_TICKER_TYPES_REQUEST_ID,
    artifact_path=CURRENT_TICKER_TYPES_ARTIFACT_PATH,
    artifact_sha256=CURRENT_TICKER_TYPES_ARTIFACT_SHA256,
    expected_rows=CURRENT_TICKER_TYPES_EXPECTED_ROWS,
)


@dataclass(frozen=True, slots=True)
class TickerTypePreviewRun:
...
def _run_ticker_type_preview_authorized(
    data_root: Path,
    *,
    workflow_id: str,
    expected_event_sha256: str,
    ...
    if workflow_id != _AUTHORIZED_WORKFLOW_ID:
        raise SilverStoreError("ticker-type preview workflow ID is not authorized")
    if TICKER_TYPE_DIM_CONTRACT.contract_id != _AUTHORIZED_CONTRACT_ID:
        raise SilverStoreError("ticker-type preview contract identity is not authorized")
    if sample_limit != authorization.sample_limit:
        raise SilverStoreError(
            f"ticker-type preview sample_limit is pinned to {authorization.sample_limit}"
        )
    if calendar_name != "XNYS":
        raise SilverStoreError("ticker_type preview calendar is pinned to XNYS")
    normalized_manifests = tuple(manifest_paths)
    if normalized_manifests != (authorization.manifest_path,):
        raise SilverStoreError("ticker_type preview is authorized for one exact manifest only")
    if expected_manifest_sha256 != authorization.manifest_sha256:
        raise SilverStoreError("ticker_type preview expected manifest SHA is not authorized")
    if expected_input_rows != authorization.expected_rows:
        raise SilverStoreError("ticker_type preview expected row count is not authorized")
    ...
    _verify_authorized_workflow_ancestry(store, current)
    if current.event_sha256 != expected_event_sha256:
        raise SilverStoreError("stale ticker-type preview expected_event_sha256")
    contract, _ = store.load_workflow_contract(workflow_id)
    if contract != TICKER_TYPE_DIM_CONTRACT:
        raise SilverStoreError("ticker_type preview workflow does not use the approved contract")
    if current.state not in {
        ...
        raise SilverStoreError(
            f"ticker_type preview cannot run from workflow state {current.state.value}"
        )
    ...
    if current.state in {WorkflowState.PREVIEW_READY, WorkflowState.AWAITING_REVIEW}:
        ...
        if current.state is WorkflowState.PREVIEW_READY:
            current = store.request_preview_review(
                ...
                actor=actor,
                created_at=_now_utc(),
                note="S2 ticker_types preview submitted for user review; full run remains gated.",
            )
        return TickerTypePreviewRun(
            workflow=current,
            build=build,
    ...
    intent = BuildIntent(
        ...
        transform_version=TICKER_TYPE_DIM_TRANSFORM_VERSION,
        git_commit=git_commit,
    ...
    current = store.record_preview_build(
        ...
        note="Registered the bounded current ticker_types preview.",
    )
    current = store.request_preview_review(
        workflow_id,
        expected_event_sha256=current.event_sha256,
...
def _materialize_preview(
    ...
    *,
    ...
    try:
        ...
        result = transform_ticker_type_batch(
            batch,
...
def _input_sample(batch: TickerTypeSourceBatch, limit: int) -> list[dict[str, object]]:
...
def _load_orphan_build_if_present(
    ...
    if build.intent != intent:
        raise SilverStoreError("orphan ticker_type preview intent differs from this run")
    ...
    for record in reversed(store.workflow_events(snapshot.workflow_id)):
        if record.event.to_state is WorkflowState.PREVIEW_READY:
            ...
            if (
                ...
                raise SilverStoreError("ticker_type preview event points to the wrong build")
            store.verify_build(build, TICKER_TYPE_DIM_CONTRACT)
            return build, document
    raise SilverStoreError("ticker_type workflow has no registered preview build")
...
def _verify_authorized_workflow_ancestry(
    ...
    """Require the pinned code-ready event while permitting an exact idempotent rerun."""

    if current.workflow_id != _AUTHORIZED_WORKFLOW_ID:
        raise SilverStoreError("ticker-type preview workflow identity changed")
    records = store.workflow_events(current.workflow_id)
    if len(records) < 3:
        raise SilverStoreError("ticker-type preview workflow has no code-ready authorization")
    authorized = records[2]
    if (
        authorized.event.sequence != 3
        or authorized.event.to_state is not WorkflowState.CODE_READY
        or authorized.event_sha256 != _AUTHORIZED_CODE_READY_EVENT_SHA256
    ):
        raise SilverStoreError("ticker-type preview does not descend from the authorized event")
    if current.state is WorkflowState.CODE_READY and (
        current.sequence != 3 or current.event_sha256 != _AUTHORIZED_CODE_READY_EVENT_SHA256
    ):
        raise SilverStoreError("ticker-type preview current code-ready event is not authorized")
...
def _load_build_inventory(
    ...
    if first.lineage_manifest_path is None or first.lineage_manifest_sha256 is None:
        raise SilverStoreError("ticker_type preview source inventory lineage is missing")
    path = safe_relative_path(root, first.lineage_manifest_path)
    try:
        content = path.read_bytes()
    except OSError as exc:
        raise SilverStoreError("cannot read ticker_type preview source inventory") from exc
    if hashlib.sha256(content).hexdigest() != first.lineage_manifest_sha256:
        raise SilverStoreError("ticker_type preview source inventory checksum mismatch")
    try:
        document = json.loads(content)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise SilverStoreError("ticker_type preview source inventory is invalid JSON") from exc
    ...
    if not all(
        ...
        raise SilverStoreError("ticker_type preview inputs do not share one source inventory")
...
def _verify_git_checkout(repo_root: Path, git_commit: str) -> None:
    ...
    try:
        ...
        raise SilverStoreError(
            "ticker_type preview code is not executing from the verified Git checkout"
        ) from exc
    try:
        ...
        raise SilverStoreError("cannot verify ticker_type preview Git checkout") from exc
    if Path(top_level).resolve() != root:
        raise SilverStoreError("ticker_type preview repo_root is not the Git top level")
    if head != git_commit:
        raise SilverStoreError("ticker_type preview Git HEAD differs from --git-commit")
    if tracked_module != module_relative:
        raise SilverStoreError("ticker_type preview module is not the verified tracked source")
    if status:
        raise SilverStoreError("ticker_type preview Git checkout is not clean")
...
def _git_output(root: Path, *arguments: str) -> str:
    ...
    if completed.returncode != 0:
        ...
        raise SilverStoreError(f"cannot verify ticker_type preview Git checkout: {detail}")
...
def _intent_parameters(
    *,
    ...
    return {
        ...
        "preview_policy_version": TICKER_TYPE_PREVIEW_POLICY_VERSION,
...
def _validate_authorized_inventory(
    ...
    *,
    ...
    if (
        ...
        raise SilverStoreError("ticker_type preview inventory is not Bronze ticker_types")
    upstream = inventory.upstream_manifests
    if len(upstream) != 1:
        raise SilverStoreError("ticker_type preview requires exactly one source manifest")
    if (
        ...
        raise SilverStoreError("ticker_type preview source manifest is not the authorized object")
    artifacts = inventory.artifacts
    if len(artifacts) != 1:
        raise SilverStoreError("ticker_type preview requires exactly one source page")
    ...
    if (
        ...
        raise SilverStoreError("ticker_type preview source page is not the authorized object")
...
def _validate_authorized_source(
    ...
    *,
    ...
    if len(batch.snapshots) != 1 or batch.page_count != 1:
        raise SilverStoreError("ticker_type preview input must be one snapshot and one page")
    snapshot = batch.snapshots[0]
    page = snapshot.pages[0]
    if snapshot.source_request_id != authorization.request_id:
        raise SilverStoreError("ticker_type preview request ID is not authorized")
    if (
        page.sequence != 0
        or page.source_path != authorization.artifact_path
        or page.source_artifact_sha256 != authorization.artifact_sha256
    ):
        raise SilverStoreError("ticker_type preview page identity is not authorized")
    if batch.row_count != expected_input_rows or batch.row_count != authorization.expected_rows:
        raise SilverStoreError("ticker_type preview input row count differs from authorization")
...
def _require_matching_existing_preview(
    ...
    *,
    ...
    if (
        ...
        or intent.contract_id != TICKER_TYPE_DIM_CONTRACT.contract_id
        or intent.transform_version != TICKER_TYPE_DIM_TRANSFORM_VERSION
        ...
        raise SilverStoreError("existing ticker_type preview does not match this exact run intent")
    if len(intent.inputs) != len(inventory.artifacts):
        raise SilverStoreError("existing ticker_type preview inputs differ from its inventory")
    for source, item in zip(intent.inputs, inventory.artifacts, strict=True):
        if (
            ...
            raise SilverStoreError("existing ticker_type preview input lineage is inconsistent")
    if build.preview is None or build.preview.full_run_inputs != intent.inputs:
        raise SilverStoreError("existing ticker_type preview full-run inventory is inconsistent")
...
def _source_raw_bytes(root: Path, inventory: SourceInventory) -> int:
    ...
    for upstream in inventory.upstream_manifests:
        ...
        try:
            ...
            raise SilverStoreError("cannot reread ticker_type source manifest for sizing") from exc
        if hashlib.sha256(content).hexdigest() != upstream.sha256:
            raise SilverStoreError("ticker_type source manifest changed during sizing")
        try:
            document = json.loads(content)
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise SilverStoreError("ticker_type source manifest is invalid during sizing") from exc
        artifacts = document.get("artifacts") if isinstance(document, dict) else None
        if not isinstance(artifacts, list) or not artifacts:
            raise SilverStoreError("ticker_type source manifest has no sizing artifacts")
        for artifact in artifacts:
            raw_bytes = artifact.get("raw_bytes") if isinstance(artifact, dict) else None
            if type(raw_bytes) is not int or raw_bytes <= 0:
                raise SilverStoreError("ticker_type source manifest raw byte count is invalid")
            total += raw_bytes
    if total <= 0:
        raise SilverStoreError("ticker_type preview source raw byte total is empty")
    return total
    ...
    *,
    ...
    snapshots = tuple(
        TickerTypeSourceSnapshot(
            ...
            pages=(
                TickerTypeSourcePage(
                    source_path=f"fixtures/ticker_types/{request_id}/page-00000.json.gz",
    ...
    result = transform_ticker_type_batch(
        TickerTypeSourceBatch(snapshots),
        build_id=build_id,
        calendar_name=calendar_name,
    )
    ...
    temporal_checks = {
        check_id: result.qa_by_id(check_id) for check_id in _TEMPORAL_FIXED_CASE_QA_CHECKS
    }
    temporal_identities = [
        {
    ...
    assertions: list[dict[str, object]] = [
        ...
        {
            "assertion_id": "no_false_temporal_drift_for_identical_adjacent_snapshots",
            "passed": len(temporal_identities) == 2
            and temporal_identities[0] == temporal_identities[1]
            and all(
                check.status.value == "passed" and check.numerator == 0 and check.denominator > 0
                for check in temporal_checks.values()
            ),
            "evidence": {
                "identities_by_capture": temporal_identities,
                "qa_results": {
                    check_id: {
                    ...
                    for check_id, check in temporal_checks.items()
                },
            },
        },
    ]
...
__all__ = ["TickerTypePreviewRun", "run_ticker_type_preview"]
