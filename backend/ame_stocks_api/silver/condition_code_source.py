"""Read-only, manifest-bound inputs for the approved condition_code_dim transform."""
...
_MANIFEST_PREFIX = "manifests/massive/condition_codes"
_ARTIFACT_PREFIX = "bronze/massive/condition_codes"


class ConditionCodeSourceError(SilverContractError):
    """Raised before transformation when manifest-bound condition_code input is unsafe."""


@dataclass(frozen=True, slots=True)
class ConditionCodeSourcePage:
    """One verified provider response page within a condition_code snapshot."""
    ...
    def __post_init__(self) -> None:
        if not self.source_path or Path(self.source_path).is_absolute():
            raise ConditionCodeSourceError("condition_code source page path must be relative")
        if not _SHA256.fullmatch(self.source_artifact_sha256):
            raise ConditionCodeSourceError("condition_code source page SHA-256 is invalid")
        if type(self.sequence) is not int or self.sequence < 0:
            raise ConditionCodeSourceError(
                "condition_code source page sequence must be nonnegative"
            )
        if not isinstance(self.source_provider_request_id, str) or not (
            self.source_provider_request_id.strip()
        ):
            raise ConditionCodeSourceError("condition_code provider request ID is missing")
        if self.source_provider_request_id != self.source_provider_request_id.strip():
            raise ConditionCodeSourceError(
                "condition_code provider request ID has surrounding whitespace"
            )
        normalized: list[Mapping[str, object]] = []
        for row in self.rows:
            if not isinstance(row, Mapping):
                raise ConditionCodeSourceError("condition_code result rows must be objects")
            try:
                detached = json.loads(
                ...
                raise ConditionCodeSourceError(
                    "condition_code result row is not safe JSON"
                ) from exc
            if not isinstance(detached, dict):  # pragma: no cover - Mapping serialized above
                raise ConditionCodeSourceError("condition_code result row is not an object")
            normalized.append(MappingProxyType(detached))
        object.__setattr__(self, "rows", tuple(normalized))


@dataclass(frozen=True, slots=True)
class ConditionCodeSourceSnapshot:
    """One immutable latest-only request observed at a specific instant."""

    source_request_id: str
    source_capture_at_utc: datetime
    pages: tuple[ConditionCodeSourcePage, ...]

    def __post_init__(self) -> None:
        if not _SHA256.fullmatch(self.source_request_id):
            raise ConditionCodeSourceError("condition_code source request ID is invalid")
        captured = self.source_capture_at_utc
        if not isinstance(captured, datetime) or captured.tzinfo is None:
            raise ConditionCodeSourceError("condition_code capture time must be timezone-aware")
        object.__setattr__(self, "source_capture_at_utc", captured.astimezone(UTC))
        pages = tuple(self.pages)
        if not pages:
            raise ConditionCodeSourceError(
                "condition_code source snapshot requires at least one page"
            )
        if tuple(page.sequence for page in pages) != tuple(range(len(pages))):
            raise ConditionCodeSourceError(
                "condition_code source pages must be contiguous and ordered"
            )
        if len({page.source_path for page in pages}) != len(pages):
            raise ConditionCodeSourceError("condition_code source page paths must be unique")
        object.__setattr__(self, "pages", pages)


@dataclass(frozen=True, slots=True)
class ConditionCodeSourceBatch:
    """Verified snapshots ready for the pure condition_code_dim transformation."""

    snapshots: tuple[ConditionCodeSourceSnapshot, ...]

    def __post_init__(self) -> None:
        snapshots = tuple(self.snapshots)
        if not snapshots:
            raise ConditionCodeSourceError("condition_code source batch cannot be empty")
        if len({item.source_request_id for item in snapshots}) != len(snapshots):
            raise ConditionCodeSourceError("condition_code source request IDs must be unique")
        object.__setattr__(
            self,
            "snapshots",
            tuple(
...
def build_condition_code_source_inventory(
    data_root: Path,
    *,
    ...
    if not paths:
        raise ConditionCodeSourceError("condition_code inventory requires at least one manifest")
    upstream: list[UpstreamManifestRef] = []
    artifacts: list[SourceInventoryItem] = []
    seen_artifacts: set[str] = set()
    for relative_manifest in paths:
        document, content, _ = _load_condition_code_manifest(root, relative_manifest)
        upstream.append(
            UpstreamManifestRef(
        ...
        for artifact in _manifest_artifacts(document):
            ...
            if relative_artifact in seen_artifacts:
                raise ConditionCodeSourceError(
                    "condition_code manifests bind the same artifact twice"
                )
            ...
            try:
                ...
                raise ConditionCodeSourceError(
                    f"cannot read condition_code Bronze artifact: {relative_artifact}"
                ) from exc
            ...
            if len(compressed) != expected_bytes:
                raise ConditionCodeSourceError("condition_code Bronze artifact byte count mismatch")
            if hashlib.sha256(compressed).hexdigest() != expected_sha:
                raise ConditionCodeSourceError("condition_code Bronze artifact checksum mismatch")
            artifacts.append(
                SourceInventoryItem(
    ...
    return SourceInventory(
        source_dataset="condition_codes",
...
def read_condition_code_source_inventory(
    data_root: Path,
    ...
    if (
        inventory.source_dataset != "condition_codes"
        or inventory.source_layer is not SourceLayer.BRONZE
    ):
        raise ConditionCodeSourceError(
            "condition_code input must be a Bronze condition_codes inventory"
        )
    manifest_paths = tuple(item.path for item in inventory.upstream_manifests)
    rebuilt = build_condition_code_source_inventory(
        data_root,
    ...
    if rebuilt.to_dict() != inventory.to_dict():
        raise ConditionCodeSourceError(
            "condition_code source inventory differs from current immutable bytes"
        )
    ...
    snapshots: list[ConditionCodeSourceSnapshot] = []
    for relative_manifest in sorted(manifest_paths):
        document, manifest_content, capture_at = _load_condition_code_manifest(
            root, relative_manifest
        )
        if hashlib.sha256(manifest_content).hexdigest() != upstream_sha_by_path[relative_manifest]:
            raise ConditionCodeSourceError("condition_code manifest changed while being read")
        request_id = _sha256_text(document.get("request_id"), "request_id")
        pages: list[ConditionCodeSourcePage] = []
        for artifact in _manifest_artifacts(document):
            relative_artifact = _required_text(artifact, "path")
            item = inventory_items.get(relative_artifact)
            if item is None:
                raise ConditionCodeSourceError(
                    "manifest page is absent from condition_code inventory"
                )
            try:
                compressed = safe_relative_path(root, relative_artifact).read_bytes()
            except OSError as exc:
                raise ConditionCodeSourceError(
                    "cannot reread condition_code Bronze artifact"
                ) from exc
            if len(compressed) != item.bytes:
                raise ConditionCodeSourceError(
                    "condition_code Bronze page changed byte count while reading"
                )
            if hashlib.sha256(compressed).hexdigest() != item.sha256:
                raise ConditionCodeSourceError(
                    "condition_code Bronze page changed checksum while reading"
                )
            try:
                raw = gzip.decompress(compressed)
            except (OSError, gzip.BadGzipFile) as exc:
                raise ConditionCodeSourceError(
                    "condition_code Bronze page is not valid gzip"
                ) from exc
            ...
            if len(raw) != expected_raw_bytes:
                raise ConditionCodeSourceError("condition_code Bronze raw byte count mismatch")
            expected_raw = _sha256_text(artifact.get("raw_sha256"), "raw_sha256")
            if hashlib.sha256(raw).hexdigest() != expected_raw:
                raise ConditionCodeSourceError("condition_code Bronze raw checksum mismatch")
            try:
                response = json.loads(raw)
            except (UnicodeDecodeError, json.JSONDecodeError) as exc:
                raise ConditionCodeSourceError(
                    "condition_code Bronze page is not valid JSON"
                ) from exc
            provider_request_id, rows = _validate_response_envelope(response)
            if len(rows) != item.row_count:
                raise ConditionCodeSourceError("condition_code response rows differ from manifest")
            pages.append(
    ...
    return ConditionCodeSourceBatch(tuple(snapshots))


def _load_condition_code_manifest(
    root: Path,
    relative_path: str,
) -> tuple[dict[str, Any], bytes, datetime]:
    if not isinstance(relative_path, str) or not relative_path.startswith(f"{_MANIFEST_PREFIX}/"):
        raise ConditionCodeSourceError("condition_code manifest is outside its canonical namespace")
    relative_manifest = Path(relative_path)
    if relative_manifest.parent.as_posix() != _MANIFEST_PREFIX:
        raise ConditionCodeSourceError(
            "condition_code manifest must use the canonical dataset directory"
        )
    ...
    try:
        ...
        raise ConditionCodeSourceError(
            f"cannot read condition_code manifest: {relative_path}"
        ) from exc
    if not isinstance(document, dict):
        raise ConditionCodeSourceError("condition_code manifest root must be an object")
    request_id = _sha256_text(document.get("request_id"), "request_id")
    if (
        document.get("status") != "complete"
        or document.get("provider") != "massive"
        or document.get("dataset") != "condition_codes"
    ):
        raise ConditionCodeSourceError(
            "condition_code manifest identity or terminal status is invalid"
        )
    expected_name = f"{request_id}.json"
    if path.name != expected_name:
        raise ConditionCodeSourceError("condition_code manifest filename does not match request ID")
    request = document.get("request")
    if not isinstance(request, dict):
        raise ConditionCodeSourceError("condition_code manifest request is missing")
    start = request.get("start")
    end = request.get("end")
    if (
        request.get("dataset") != "condition_codes"
        ...
        raise ConditionCodeSourceError(
            "condition_code manifest is not the approved latest-only request"
        )
    try:
        date.fromisoformat(start)
    except ValueError as exc:
        raise ConditionCodeSourceError("condition_code request date label is invalid") from exc
    ...
    if created_at > capture_at:
        raise ConditionCodeSourceError("condition_code manifest completion precedes creation")
    ...
    if actual_sequences != expected_sequences:
        raise ConditionCodeSourceError("condition_code manifest page sequences are not contiguous")
    for artifact in artifacts:
        ...
        if relative_artifact.parent.as_posix() != expected_parent:
            raise ConditionCodeSourceError(
                "condition_code artifact path does not match its request"
            )
        if relative_artifact.name != f"page-{sequence:05d}.json.gz":
            raise ConditionCodeSourceError(
                "condition_code artifact filename does not match its sequence"
            )
        if artifact.get("content_type") != "application/json":
            raise ConditionCodeSourceError("condition_code artifact content type is not JSON")
        ...
        if artifact.get("is_last") is not (sequence == len(artifacts) - 1):
            raise ConditionCodeSourceError(
                "condition_code manifest page termination marker is invalid"
            )
...
def _manifest_artifacts(document: Mapping[str, object]) -> tuple[dict[str, Any], ...]:
    ...
    if not isinstance(artifacts, list) or not artifacts:
        raise ConditionCodeSourceError("complete condition_code manifest has no artifacts")
    if not all(isinstance(item, dict) for item in artifacts):
        raise ConditionCodeSourceError("condition_code manifest artifacts must be objects")
    return tuple(artifacts)


def _validate_response_envelope(response: object) -> tuple[str, tuple[dict[str, Any], ...]]:
    if not isinstance(response, dict) or response.get("status") != "OK":
        raise ConditionCodeSourceError("condition_code response envelope status is invalid")
    provider_request_id = response.get("request_id")
    if not isinstance(provider_request_id, str) or not provider_request_id.strip():
        raise ConditionCodeSourceError("condition_code response request ID is missing")
    if provider_request_id != provider_request_id.strip():
        raise ConditionCodeSourceError(
            "condition_code response request ID has surrounding whitespace"
        )
    results = response.get("results")
    if not isinstance(results, list) or not all(isinstance(item, dict) for item in results):
        raise ConditionCodeSourceError(
            "condition_code response results must be an array of objects"
        )
    count = response.get("count")
    if count is not None and (type(count) is not int or count < 0 or count != len(results)):
        raise ConditionCodeSourceError("condition_code response count does not match results")
...
def _required_text(document: Mapping[str, object], key: str) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise ConditionCodeSourceError(f"condition_code source {key} must be a nonempty string")
    return value


def _sha256_text(value: object, label: str) -> str:
    if not isinstance(value, str) or not _SHA256.fullmatch(value):
        raise ConditionCodeSourceError(f"condition_code {label} must be a lowercase SHA-256")
    return value


def _native_nonnegative_int(value: object, label: str) -> int:
    if type(value) is not int or value < 0:
        raise ConditionCodeSourceError(f"condition_code {label} must be a nonnegative native int")
    return value


def _utc_datetime(value: object, label: str) -> datetime:
    if not isinstance(value, str):
        raise ConditionCodeSourceError(f"condition_code {label} must be an ISO timestamp")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise ConditionCodeSourceError(f"condition_code {label} is not an ISO timestamp") from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise ConditionCodeSourceError(f"condition_code {label} must be timezone-aware")
